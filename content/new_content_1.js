...
async function processYouTubeWatchPage() {
  ...
  // Try every known YouTube title selector
  const titleEl =
  ...
  const rawText = [titleEl, descEl]
    .filter(Boolean)
    .map(el => el.innerText || el.textContent)
    .join(' ')
    .trim();

  console.log(`[SpoilerShield] Watch page text (${rawText.length} chars): "${rawText.slice(0, 80)}"`);

  // If title not found yet, retry after 1.5s
  if (!rawText) {
    console.warn('[SpoilerShield] No title found yet, retrying in 1.5s.');
    setTimeout(() => { processYouTubeWatchPage._lastUrl = null; processYouTubeWatchPage(); }, 1500);
    return;
  }

  const matchedShow = findMentionedShow(rawText.toLowerCase());
  if (!matchedShow) {
    console.log('[SpoilerShield] Watch page title has no protected show.');
    return;
  }

  console.log(`[SpoilerShield] Watch page mentions "${matchedShow}" — checking Gemini…`);

  const isSpoiler = await askGemini(rawText, matchedShow);
  if (isSpoiler) {
    console.log('[SpoilerShield] Confirmed spoiler — injecting overlay…');
