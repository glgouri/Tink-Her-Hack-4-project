...
function blurYouTubePlayer(showName) {
  // Remove any existing overlay first (in case of re-navigation)
  const existing = document.getElementById('ss-yt-player-overlay');
  if (existing) existing.remove();

  // Try multiple selectors — YouTube's DOM varies by page layout
  const player =
  ...
  // Mute and pause the video element directly
  if (player) {
    player.muted  = true;
    player.pause();
  ...
  // Build overlay — use fixed positioning anchored to viewport
  // so YouTube's own z-index/overflow/stacking cannot hide it
  const overlay = document.createElement('div');
  overlay.id = 'ss-yt-player-overlay';
  ...
  // Attach directly to body — completely outside YouTube's DOM tree
  document.body.appendChild(overlay);
  console.log(`[SpoilerShield] Full-screen overlay injected for: "${displayName}"`);
}
